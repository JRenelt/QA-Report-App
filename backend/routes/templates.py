...
async def get_project_template_csv(current_user: dict = Depends(get_current_user)):
    ...
    # Beispieldaten (3 Bereiche, 6 Testfälle)
    examples = [
        ['TEST001', 'Login-Funktion testen', 'UI/UX Test', 'Prüfen ob Login funktioniert', '1', 'User wird eingeloggt', 'pending'],
        ['TEST002', 'Navigation testen', 'UI/UX Test', 'Alle Menüpunkte anklicken', '2', 'Navigation funktioniert', 'pending'],
        ['TEST003', 'Datenspeicherung testen', 'Funktionalität', 'Daten in DB speichern', '1', 'Daten werden gespeichert', 'pending'],
        ['TEST004', 'API-Endpoints testen', 'Funktionalität', 'Alle REST-APIs prüfen', '1', 'APIs antworten korrekt', 'pending'],
        ['TEST005', 'Ladezeiten messen', 'Performance', 'Seitenladezeit unter 2s', '2', 'Seite lädt schnell', 'pending'],
        ['TEST006', 'Stress-Test durchführen', 'Performance', '100 gleichzeitige User', '3', 'System bleibt stabil', 'pending'],
    ]
