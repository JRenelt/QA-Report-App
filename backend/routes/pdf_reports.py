...
async def generate_pdf_report(
    ...
    # Footer-Funktion mit "Seite X von Y" - verwenden wir NumberedCanvas
    from reportlab.pdfgen.canvas import Canvas
    
    class NumberedCanvas(Canvas):
        """Custom Canvas für Seitenzahlen mit 'von Y'"""
        ...
        def save(self):
            """Überschreibe save() um Seitenzahlen zu aktualisieren"""
        ...
        def draw_page_number(self, page_count):
            """Zeichne Fusszeile mit Seitenzahl"""
            self.saveState()
            self.setFont('Helvetica', 8)
            self.setFillColor(colors.HexColor('#7F8C8D'))
            # Copyright links
            self.drawString(1.5*cm, 1.2*cm, "© 2025 • Jörg Renelt • Hamburg")
            # Seitenzahl rechts
            page_num = self.getPageNumber()
            self.drawRightString(A4[0] - 1.5*cm, 1.2*cm, f"Seite {page_num} von {page_count}")
            self.restoreState()
    
    def add_page_footer_final(canvas, doc):
        """Dummy-Footer-Funktion (NumberedCanvas macht die eigentliche Arbeit)"""
        pass
    ...
    # Combine both tables - Linke GANZ LINKS, rechte GANZ RECHTS
    # Nutzbare Breite: 18cm = 7.087 inch
    # Mehr Abstand in der Mitte, damit rechte Tabelle weiter rechts ist
    combined_info_data = [[left_table, '', right_table]]
    combined_table = Table(combined_info_data, colWidths=[3.8*inch, 0.7*inch, 2.587*inch])  # Spacer 0.7 inch
    ...
    def create_html_style_card(number, label, border_color, bg_color):
        """Erstellt VERTIKALE Karte (HOCH): abgerundete Ecken, farbiger Rahmen, transparenter Hintergrund - wie BILD 3"""
        # Zahl (30pt, fett, dunkel) - KLEINER für vertikales Layout
        num_para = Paragraph(
            f"<font size=30 color='#333333'><b>{number}</b></font>",
            ParagraphStyle('CardNum', parent=body_style, alignment=TA_CENTER, leading=36)
        )
        # Label (kleinere Schrift, grau)
        label_para = Paragraph(
            f"<font size=8 color='#555555'>{label}</font>",
            ParagraphStyle('CardLabel', parent=body_style, alignment=TA_CENTER, leading=10, spaceBefore=5)
        )
        ...
        mini_table.setStyle(TableStyle([
            ...
            ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
            ('TOPPADDING', (0, 0), (0, 0), 25),    # Zahl: viel Abstand oben
            ('BOTTOMPADDING', (0, 0), (0, 0), 8),   # Zahl: weniger unten
            ('TOPPADDING', (0, 1), (0, 1), 8),      # Label: wenig oben
            ('BOTTOMPADDING', (0, 1), (0, 1), 25),  # Label: viel Abstand unten
    ...
    # Outer Table: 5 Karten nebeneinander - VERTIKAL mit SICHTBAREM ABSTAND!
    # Nutzbare Breite: 18cm = 7.087 inch  
    # 5 Karten à 1.0 inch = 5.0 inch, bleiben 2.087 inch für Abstände
    # 4 Abstände zwischen 5 Karten = 0.52 inch pro Abstand (DEUTLICH SICHTBAR!)
    # Karten: VERTIKAL = schmal (1.0 inch) x HOCH (2.0 inch)
    card_table = Table(card_data, colWidths=[1.0*inch]*5, rowHeights=[2.0*inch])
    card_table.setStyle(TableStyle([
        ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
        ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
        ('LEFTPADDING', (0, 0), (-1, -1), 0.26*inch),   # Halber Abstand = 0.26 inch
        ('RIGHTPADDING', (0, 0), (-1, -1), 0.26*inch),  # Halber Abstand = 0.26 inch → gesamt 0.52 inch!
        ('TOPPADDING', (0, 0), (-1, -1), 0),
        ('BOTTOMPADDING', (0, 0), (-1, -1), 0)
    ]))
